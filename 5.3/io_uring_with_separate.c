...
static struct kmem_cache * scheduler_cachep;
...
struct schedule_info{
	struct task_struct *global_sqo_thread;
...
static int sq_do_work(struct work_struct *work)
{
		...
		all_fixed = true;
		do {
			if (all_fixed && io_sqe_needs_user(sq_w->sqes[i].sqe))
				all_fixed = false;

			i++;
			if (i == ARRAY_SIZE(sq_w->sqes))
			{
			break;
			}
		} while (io_get_sqring(ctx_ptr, &sq_w->sqes[i]));
		...
		*/
		...
		inflight += io_submit_sqes(ctx_ptr, &sq_w->sqes[0], i, cur_mm != NULL,
						mm_fault);
...
static int io_sq_thread(void *data)
{
	...
	printk(KERN_ERR "USING_OLD_THREAD_FUNCTION_NOW\n");
...
static void io_sq_thread_stop(struct io_ring_ctx *ctx)
{
	...
	*/
	...
	if (si->head_sqo_thread) {
		wait_for_completion(&si->head_sqo_thread_started);
		kthread_park(si->head_sqo_thread);
		kthread_stop(si->head_sqo_thread);
			       ...
			       struct io_uring_params *p)
{
	...
	if (ctx->flags & IORING_SETUP_SQPOLL) {
		...
		if (p->flags & IORING_SETUP_SQ_AFF) {
			...
			*/
			if (!si->head_sqo_thread) {
				printk(KERN_ERR "Got into the right thread.");
		...
		if (IS_ERR(si->head_sqo_thread)) {
			ret = PTR_ERR(si->head_sqo_thread);
			si->head_sqo_thread = NULL;
			goto err;
		}
		/*
...
static void init_once(void *foo)
{
		...
		struct schedule_info *si = (struct schedule_info *) foo;
		...
		*/
		...
		/*
		si->global_sqo_thread = kthread_create_on_cpu(io_sq_thread,
		  ctx, cpu,
			"io_uring-sq");
		*/
...
{
	printk(KERN_ERR "GOT INTO IO_URING INIT\n");
	req_cachep = KMEM_CACHE(io_kiocb, SLAB_HWCACHE_ALIGN | SLAB_PANIC);
	scheduler_cachep = kmem_cache_create("scheduler_cache", sizeof(struct schedule_info),
	  0, SLAB_RECLAIM_ACCOUNT|SLAB_MEM_SPREAD,
		init_once);

	if (scheduler_cachep == NULL) {
		return -ENOMEM;
	}
