...
static struct schedule_info *si;
...
static int io_submit_sqes_ctx_list(struct list_head *ctx_list,
				...
				bool async)
{
	...
	list_for_each_entry_rcu(context_ptr, &si->si_ctx->ctx_list, ctx_list)
	{
		...
		if (test_bit(0, &ctx_ptr->sq_check_overflow)) {
			if (!list_empty(&ctx_ptr->cq_overflow_list) &&
					!io_cqring_overflow_flush(ctx_ptr, false))
				return -EBUSY;
		}
		...
		unsigned int nr;
		nr = min3(context_ptr->to_submit, ctx_ptr->sq_entries, io_sqring_entries(ctx_ptr));

		mm = &context_ptr->cur_mm;

		if (!percpu_ref_tryget_many(&ctx_ptr->refs, nr))
			return -EAGAIN;
		...
		ctx_ptr->ring_fd = ring_fd;
		ctx_ptr->ring_file = ring_file;
		...
		for (i = 0; i < nr; i++) {
			...
			req = io_get_req(ctx_ptr, statep);
			if (unlikely(!req)) {
				if (!submitted)
					submitted = -EAGAIN;
				break;
			}
			if (!io_get_sqring(ctx_ptr, req, &sqe)) {
				__io_req_do_free(req);
				break;
			}
	...
	fail_req:
			...
			if (io_op_defs[req->opcode].needs_mm && !*mm) {
				mm_fault = mm_fault || !mmget_not_zero(ctx_ptr->sqo_mm);
			...
			trace_io_uring_submit_sqe(ctx_ptr, req->opcode, req->user_data,
							true, async);
		...
		if (unlikely(submitted != nr)) {
			...
			percpu_ref_put_many(&ctx_ptr->refs, nr - ref_used);
		}
		if (link)
...
static int io_submit_sqes(struct io_ring_ctx *ctx, unsigned int nr,
			  struct file *ring_file, int ring_fd, struct mm_struct **mm, bool async)
{
	struct io_submit_state state, *statep = NULL;
	struct io_kiocb *link = NULL;
	int i, submitted = 0;
...
static int io_sq_thread_ctx_list(void *data)
{
	...
	while (!kthread_should_park()) {
		...
		context_ptr->ret = io_submit_sqes_ctx_list(&si->si_ctx->ctx_list, NULL, -1, true);
		//trace_sq_thread_after_submit(context_ptr->ret);
		mutex_unlock(&ctx_ptr->uring_lock);
		timeout = jiffies + 5 * ctx_ptr->sq_thread_idle;
		rcu_read_unlock();
	}
